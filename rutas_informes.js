...
router.get('/hembras-descartar', async (req, res) => {
  ...
  res.json({
    criterios: { ciclo_max: cicloMax, nacidos_vivos_min: nvMin },
...
router.post('/bandas', async (req, res) => {
  const { nombre, fecha_servicio, fecha_parto, observaciones } = req.body;
  if (!nombre) return res.status(400).json({ error: 'El nombre de la banda es obligatorio' });
  try {
    const { rows } = await pool.query(
      `INSERT INTO bandas (nombre, fecha_servicio, fecha_parto, observaciones)
       VALUES ($1,$2,$3,$4) RETURNING *`,
      [nombre.trim(), fecha_servicio||null, fecha_parto||null, observaciones||null]);
    res.json(rows[0]);

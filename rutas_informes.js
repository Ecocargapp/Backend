...
// Renombrar una banda
router.patch('/bandas/:id', async (req, res) => {
  const { nombre } = req.body;
  if (!nombre) return res.status(400).json({ error: 'El nombre es obligatorio' });
  try {
    const { rows } = await pool.query(
      'UPDATE bandas SET nombre=$1 WHERE id=$2 RETURNING *', [nombre.trim(), Number(req.params.id)]);
    if (!rows.length) return res.status(404).json({ error: 'Banda no encontrada' });
    res.json(rows[0]);
  } catch { res.status(400).json({ error: 'Ya existe una banda con ese nombre' }); }
});

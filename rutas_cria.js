...
router.get('/cerdas', async (req, res) => {
  ...
  // ult_registro = el momento más reciente en que se registró un movimiento
  // (servicio, parto o destete) para esa cerda. Las recién tocadas suben al tope.
  let sql = `
    SELECT c.*, r.nombre AS raza,
           GREATEST(
             COALESCE((SELECT MAX(creado_en) FROM servicios s WHERE s.cerda_id=c.id), '1900-01-01'),
             COALESCE((SELECT MAX(creado_en) FROM partos p WHERE p.cerda_id=c.id), '1900-01-01'),
             COALESCE((SELECT MAX(creado_en) FROM destetes d WHERE d.cerda_id=c.id), '1900-01-01')
           ) AS ult_registro
  ...
  // Orden: primero las de movimiento más reciente; las que nunca han tenido
  // movimiento (ult_registro = 1900) quedan al final, ordenadas por arete.
  sql += ` ORDER BY ult_registro DESC, c.arete`;

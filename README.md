...
## Módulo de ceba / engorde (Etapa 6)

Inventario por granja (Barrancas, Barro), movimientos, consumo y liquidación.
- Granjas: /api/ceba/granjas (GET/POST).
- Inventario: /api/ceba/inventario/:granjaId (ingresos - muertes - ventas + consumo).
- Movimientos: /api/ceba/movimientos (ingreso/muerte/venta). Valida no dejar inventario negativo.
- Consumo: /api/ceba/consumo (bultos de 40 kg).
- Liquidación: /api/ceba/liquidar (peso de salida → conversión y costo/kg).
  · Conversión = kg alimento ÷ kg producidos (ganancia de peso).
  · Costo/kg = (alimento + lechones) ÷ kg producidos.
  · Con guardar=true, archiva la liquidación en liquidaciones_ceba.

---

## Integración cría→ceba y ventas parciales (Etapa 7)

- Módulo compartido costeo_banda.js: calcula costo por lechón de una banda.
- Lotes destetados (lotes_destete): se crean desde una banda, heredan lechones,
  peso total (suma de camadas) y costo por lechón. Se trasladan a Barro/Barrancas
  generando el ingreso automáticamente.
  · GET /api/ceba/lote-preview/:bandaId — valores calculados para confirmar.
  · POST /api/ceba/lotes — crear lote (valores confirmados/ajustados).
  · POST /api/ceba/lotes/:id/trasladar — genera ingreso en ceba.
- Ventas parciales con costeo: cada venta prorratea el alimento por cabeza sobre
  el inventario actual, descontando lo ya cobrado en ventas previas (no cobra doble).
  Guarda alimento asignado, costo lechones, kg producidos, conversión y costo/kg.

---

## Rediseño de ceba por lote (Etapa 8)

El lote es la unidad central. Consumo y movimientos (muerte/venta) se ligan al lote.
- El alimento se registra por lote (lote_id en consumo_ceba).
- Muertes/ventas indican de qué lote salen (lote_id en movimientos_ceba).
- El costeo de venta parcial prorratea el alimento DEL LOTE (no de la granja),
  descontando lo ya cobrado. Así lotes que conviven en una granja no se subsidian.
- Liquidación por lote: POST /api/ceba/liquidar con lote_id.
- El ingreso ya NO es un movimiento manual: se genera al trasladar el lote.
- Nuevos endpoints: /lotes-granja/:granjaId, /lote-inventario/:loteId.

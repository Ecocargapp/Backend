...
-- ============================================================
--  REDISEÑO POR LOTE (Etapa 8)
--  El lote es la unidad central: consumo y movimientos se ligan al lote.
-- ============================================================
-- El lote ahora tiene inventario propio y estado dentro de la granja.
ALTER TABLE lotes_destete ADD COLUMN IF NOT EXISTS peso_entrada_prom NUMERIC(10,2);
ALTER TABLE lotes_destete ADD COLUMN IF NOT EXISTS activo BOOLEAN NOT NULL DEFAULT TRUE;

-- Consumo y movimientos se ligan al lote (además de la granja)
ALTER TABLE consumo_ceba ADD COLUMN IF NOT EXISTS lote_id INTEGER REFERENCES lotes_destete(id);
ALTER TABLE movimientos_ceba ADD COLUMN IF NOT EXISTS lote_id INTEGER REFERENCES lotes_destete(id);
CREATE INDEX IF NOT EXISTS idx_consumo_ceba_lote ON consumo_ceba(lote_id);
CREATE INDEX IF NOT EXISTS idx_mov_ceba_lote ON movimientos_ceba(lote_id);

-- ============================================================
--  LOTES POR DESTETE (Etapa 9): agrupar destetes sin depender de banda
-- ============================================================
-- Marca qué destetes ya fueron asignados a un lote (para no duplicar lechones).
ALTER TABLE destetes ADD COLUMN IF NOT EXISTS lote_id INTEGER REFERENCES lotes_destete(id);
CREATE INDEX IF NOT EXISTS idx_destetes_lote ON destetes(lote_id);
-- El lote ya no necesita banda obligatoria (puede armarse por fechas o selección).
ALTER TABLE lotes_destete ALTER COLUMN banda_id DROP NOT NULL;

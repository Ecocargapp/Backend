...
router.get('/costeo/:bandaId', async (req, res) => {
  ...
  const bultos_lactancia_banda = Number(lact.bultos);
  const costo_lactancia_banda = Number(lact.costo);
  ...
  res.json({
    ...
    lactancia: {
      ...
      metodo: 'consumo real ingresado por banda'
    },
    ...
    nota: 'Costo solo de alimento (gestación prorrateada + lactancia real). No incluye sanidad, mano de obra ni instalaciones.'
  });
});
